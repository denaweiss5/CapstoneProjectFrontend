...
import { Divider, Header, Segment, Form, Input } from "semantic-ui-react";
...
class Diaries extends React.Component {
  ...
  render() {
    ...
    let totalMealCals;
    const exerciseCals = this.props.specificExercises.map(
      (exercise) => exercise.calories_burned
    );
    const mealCals = this.props.specificMeals.map(
      (exercise) => exercise.calories
    );
